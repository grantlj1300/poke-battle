...
export function useBattleSequence(sequence, player, opponent) {
  ...
  const [playerHealth, setPlayerHealth] = useState(player[0].maxHP);
  const [playerMoves, setPlayerMoves] = useState(player[0].moves);
  const [playerAnimation, setPlayerAnimation] = useState("static");
  const [opponentHealth, setOpponentHealth] = useState(opponent[0].maxHP);
  const [opponentMoves, setOpponentMoves] = useState(opponent[0].moves);
  ...
  useEffect(() => {
    ...
    if (mode && !inSequence) {
      const attacker = turn === 0 ? player[0] : opponent[0];
      const receiver = turn === 0 ? opponent[0] : player[0];
